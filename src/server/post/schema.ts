...
export const getPostItemRequestSchema = (
    ...
    let slug = z.string().max(250, { message: 'slug不得超过250个字符' }).optional();
    if (!isNil(slugUniqueValidator)) {
        slug = slug.refine(slugUniqueValidator, { message: 'slug必须是唯一的,请重新设置' });
    }
    return z
        .object({
            title: z
                .string()
                .min(1, { message: '标题不得少于1个字符' })
                .max(200, { message: '标题不得超过200个字符' }),
            body: z.string().min(1, { message: '内容不得少于1个字符' }),
            summary: z.string().max(300, { message: '摘要不得超过300个字符' }).optional(),
            keywords: z.string().max(200, { message: '描述不得超过200个字符' }).optional(),
            description: z
                .string()
                .max(300, {
 ...
 * 文章分页查询响应数据结构
 */
export const postPaginateResponseSchema = z
    .object({
